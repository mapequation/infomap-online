...
export default () => (
  <>
    ...
    <p>
      ...
      a network. For a given network partition <TeX>M</TeX>, the map equation specifies the
      theoretical limit <TeX>L(M)</TeX> of how concisely we can describe the trajectory of this
      random walk.
    </p>
    <p>
    ...
    <p>
      ...
      random walker's movements. The module codebooks have codewords for nodes within each module
      (and exit codes to leave the module), which are derived from the node visit/exit frequencies
      of the random walker. The index codebook has codewords for the modules, which are derived from
      the module switch rates of the random walker. Therefore, the average length of the code
      describing a step of the random walker is the average length of codewords from the index
      codebook and the module codebooks weighted by their rates of use:
    </p>
    <TeX
    ...
    <p>
      The description length for module partition <TeX>M</TeX>. For module partition <TeX>M</TeX> of{" "}
      <TeX>n</TeX> nodes into <TeX>m</TeX> modules, the lower bound of the average length of the
      code describing a step of the random walker.
    </p>
    ...
    <p>
      ...
      is given by the total use rate of the <TeX>m</TeX> module codebooks. For module <TeX>i</TeX>,
      this is given by the fraction of time the random walker spends in module <TeX>i</TeX>, which
      is given by the total probability that any node in the module is visited, plus the probability
      that it exits the module and the exit message is used.
    </p>
    ...
    <p>
      ...
      entropy of the relative rates at which the random walker exits module <TeX>i</TeX> and visits
      each node in module <TeX>i</TeX> measures the smallest average codeword length that is
