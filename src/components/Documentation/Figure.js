...
const Figure = ({ id }) => {
  switch (id) {
      ...
      return (
        <figure id={id}>
          ...
          <figcaption>
            ...
            With the <code>*Multilayer</code> heading, links between layers are 
            explicitly defined, here visualized to highlight how the same links would 
            be automatically generated using inter-layer links (
            <FigLink id="FigureMultilayerNetworkIntraInter" />) or only intra-layer 
            links (<FigLink id="FigureMultilayerNetworkIntra" /> using relax
            rate <InlineMath>r = 0.4</InlineMath>). This multilayer network is also 
            represented as a state network in <FigLink id="FigureStateNetwork" />.
          </figcaption>
        </figure>
      );
    case "FigureMultilayerNetworkIntraInter":
      return (
        <figure id={id}>
          <img src={imgMultilayerNetworkIntraInter} alt="Multilayer network in intra-inter format" />
          <figcaption>
            <strong>Figure {figNumber(id)}.</strong> A multilayer network with five physical
            nodes <InlineMath>i,\dotsc,m</InlineMath> in two layers,{" "}
            <InlineMath>\alpha</InlineMath> and <InlineMath>\beta</InlineMath>, using the 
            <code>*Intra</code> and <code>*Inter</code> format. The relative weights of the 
            links between layers in this format defines the probability for the random walker to 
            switch layer for each node individually. However, the jump between layers are not encoded 
            as they occur within the same physical nodes, so each inter-layer link is expanded to the 
            next possible steps of the random walker, as visualised in{" "}
            <FigLink id="FigureMultilayerNetworkFull" />. This network also corresponds to the multilayer 
            network in <FigLink id="FigureMultilayerNetworkIntra" /> where the links between layers are 
            generated automatically using relax rate <InlineMath>r = 0.4</InlineMath>.
          </figcaption>
        </figure>
      );
    case "FigureMultilayerNetworkIntra":
      return (
        <figure id={id}>
          <img src={imgMultilayerNetworkIntra} alt="Multilayer network using inter-layer relaxation" />
          <figcaption>
            <strong>Figure {figNumber(id)}.</strong> A multilayer network with five physical
            nodes <InlineMath>i,\dotsc,m</InlineMath> in two layers,{" "}
            <InlineMath>\alpha</InlineMath> and <InlineMath>\beta</InlineMath>, with 
            links between layers generated by a relax rate <InlineMath>r</InlineMath>.
            Without any explicit inter-layer links defined, the flow between layers 
            through the common physical nodes can be modelled with a relax
            ...
            <InlineMath>r = 0.4</InlineMath>, this network is equivalent with both the
            multilayer networks in <FigLink id="FigureMultilayerNetworkFull" /> and{" "}
            <FigLink id="FigureMultilayerNetworkIntraInter" /> and the state network 
            in <FigLink id="FigureStateNetwork" />.
          </figcaption>
        </figure>
      );
